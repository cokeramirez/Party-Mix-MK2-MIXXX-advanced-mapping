...
var NumarkPartyMix = function() {
    ...
    var lastMovementTime = { 1: 0, 2: 0 };
    var lastScratchExitTime = { 1: 0, 2: 0 }; // <--- ESTA ES LA QUE FALTABA
    ...
    this.onTrackLoaded = function(value, group, control) {
        if (value === 1) {
            ...
            // Si estaba en modo scratch por inercia, lo desactivamos al cargar track
            if (engine.isScratching(deckNum)) {
                engine.scratchDisable(deckNum);
            }

            // Reset de Audio y efectos
            for (var i = 1; i <= 2; i++) {
